import { useState } from "react";
import { Link } from "react-router-dom";
import {
  useGreenPoints, useMyContributions, useMyGreenImpact, useMyImpact, useMyPendingDonations, useMyRequests, useQRCode,
} from "../api/queries";
import { useAuthStore } from "../store/auth";
import { IMPACT_CATEGORY_ICONS, IMPACT_CATEGORY_LABELS, type ImpactCategory, type ImpactContribution } from "../api/types";
import { Button, Card, CenterLoading, DemoBadge, EmptyState, TopBar } from "../components/ui";
import { formatKg, formatNumber, formatToman, toJalali } from "../lib/format";

const CATEGORY_ORDER: ImpactCategory[] = ["ENVIRONMENT", "SOCIAL", "EMPLOYMENT", "LOCAL"];
const CATEGORY_BAR_COLORS: Record<ImpactCategory, string> = {
  ENVIRONMENT: "bg-brand-500",
  SOCIAL: "bg-rose-400",
  EMPLOYMENT: "bg-amber-400",
  LOCAL: "bg-sky-400",
};

// نگاشت نام سطح (از IMPACT_TIERS بک‌اند) به مرحلهٔ رشد درخت — ۰ تا ۴.
const TIER_STAGE: Record<string, number> = {
  "تازه‌وارد": 0, "شروع‌کننده سبز": 1, "همراه طبیعت": 2, "سفیر سبز": 3, "قهرمان اثر سبز": 4,
};

const TREE_FOLIAGE: Record<number, { cx: number; cy: number; r: number }[]> = {
  0: [{ cx: 60, cy: 84, r: 9 }],
  1: [{ cx: 60, cy: 76, r: 13 }, { cx: 48, cy: 84, r: 8 }, { cx: 72, cy: 84, r: 8 }],
  2: [{ cx: 60, cy: 64, r: 16 }, { cx: 41, cy: 78, r: 12 }, { cx: 79, cy: 78, r: 12 }, { cx: 60, cy: 90, r: 11 }],
  3: [{ cx: 60, cy: 52, r: 18 }, { cx: 38, cy: 68, r: 14 }, { cx: 82, cy: 68, r: 14 }, { cx: 48, cy: 90, r: 13 }, { cx: 72, cy: 90, r: 13 }],
  4: [
    { cx: 60, cy: 40, r: 20 }, { cx: 34, cy: 58, r: 16 }, { cx: 86, cy: 58, r: 16 },
    { cx: 46, cy: 82, r: 15 }, { cx: 74, cy: 82, r: 15 }, { cx: 60, cy: 94, r: 14 },
  ],
};

/** درخت اثر سبز — با ارتقای سطح شهروند، تاج درخت بزرگ‌تر و پرشاخ‌وبرگ‌تر می‌شود. عنصر بصری/هویتی صفحه. */
function GreenTree({ stage }: { stage: number }) {
  const foliage = TREE_FOLIAGE[stage] ?? TREE_FOLIAGE[0];
  return (
    <svg viewBox="0 0 120 150" className="w-28 h-32 mx-auto" aria-hidden="true">
      <ellipse cx="60" cy="140" rx="38" ry="7" fill="#ffffff" opacity="0.12" />
      <rect x="55" y="94" width="10" height="42" rx="3" fill="#ffffff" opacity="0.55" />
      {foliage.map((f, i) => (
        <circle
          key={i}
          cx={f.cx}
          cy={f.cy}
          r={f.r}
          fill="#ffffff"
          opacity={0.22 + (i % 3) * 0.08}
          className="animate-fade-up"
          style={{ animationDelay: `${i * 0.08}s` }}
        />
      ))}
      {stage === 4 && (
        <>
          <circle cx="24" cy="30" r="2" fill="#ffffff" opacity="0.8" />
          <circle cx="96" cy="24" r="2.5" fill="#ffffff" opacity="0.7" />
          <circle cx="100" cy="50" r="1.6" fill="#ffffff" opacity="0.8" />
        </>
      )}
    </svg>
  );
}

function ReceiptModal({ contribution, onClose }: { contribution: ImpactContribution; onClose: () => void }) {
  const { data: qr, isLoading } = useQRCode(contribution.tracking_code);

  function handleDownload() {
    if (!qr) return;
    const a = document.createElement("a");
    a.href = qr;
    a.download = `sabzino-green-impact-${contribution.tracking_code}.png`;
    document.body.appendChild(a);
    a.click();
    a.remove();
  }

  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 px-4 pb-4 sm:pb-0">
      <div className="w-full max-w-sm bg-white rounded-3xl p-6 relative animate-impact-sheet max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-center gap-1.5 mb-1">
          <span className="text-base">🌱</span>
          <p className="text-sm font-extrabold text-ink-900">رسید دیجیتال مشارکت</p>
        </div>
        <p className="text-[11px] text-ink-500 text-center mb-4">اثر سبز من — سبزینو</p>

        <div className="flex flex-col items-center text-center">
          {isLoading ? (
            <CenterLoading />
          ) : qr ? (
            <img src={qr} alt="QR رسید" className="w-32 h-32 rounded-xl border border-brand-100" />
          ) : null}
          <p className="text-[11px] text-ink-500 mt-2">کد پیگیری: {contribution.tracking_code}</p>
        </div>

        <div className="mt-4 flex flex-col gap-2.5 text-sm">
          <Row label="طرح" value={`${contribution.project_icon} ${contribution.project_title}`} />
          <Row label="دسته‌بندی" value={IMPACT_CATEGORY_LABELS[contribution.project_category]} />
          <Row label="مبلغ مشارکت" value={`${formatToman(contribution.amount)} تومان`} />
          {contribution.waste_value_snapshot && (
            <Row label="ارزش کل تحویل" value={`${formatToman(contribution.waste_value_snapshot)} تومان`} />
          )}
          {contribution.request_code && <Row label="کد درخواست" value={contribution.request_code} />}
          <Row label="تاریخ" value={toJalali(contribution.created_at)} />
        </div>

        <div className="flex flex-col gap-2 mt-5">
          <Button full variant="secondary" onClick={handleDownload} disabled={!qr}>
            دانلود فایل رسید (QR)
          </Button>
          <Button full onClick={onClose}>
            بستن
          </Button>
        </div>
      </div>
    </div>
  );
}

function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between border-b border-brand-50 pb-2">
      <span className="text-ink-500 text-xs">{label}</span>
      <span className="text-ink-900 font-medium text-xs">{value}</span>
    </div>
  );
}

function daysLeft(deadline: string): number {
  return Math.max(0, Math.ceil((new Date(deadline).getTime() - Date.now()) / 86400000));
}

export default function GreenImpact() {
  const user = useAuthStore((s) => s.user);
  const { data: impact, isLoading: impactLoading } = useMyGreenImpact();
  const { data: myImpact } = useMyImpact();
  const { data: points } = useGreenPoints();
  const { data: requests } = useMyRequests();
  const { data: contributions, isLoading: contributionsLoading } = useMyContributions();
  const { data: pendingDonations } = useMyPendingDonations();
  const [receipt, setReceipt] = useState<ImpactContribution | null>(null);
  const [shared, setShared] = useState(false);

  const economicValue = (requests || []).reduce(
    (sum, r) => sum + (r.weighing ? Number(r.weighing.total_value) : 0),
    0,
  );

  const tierPct = impact
    ? impact.tier.next_threshold
      ? Math.min(100, Math.round((impact.tier.deliveries / impact.tier.next_threshold) * 100))
      : 100
    : 0;
  const stage = impact ? TIER_STAGE[impact.tier.name] ?? 0 : 0;

  async function handleShare() {
    if (!impact) return;
    const text =
      `من در سبزینو ${formatKg(myImpact?.total_kg_recycled || 0)} کیلوگرم بازیافت کردم و ` +
      `${formatToman(impact.total_contributed)} تومان از ارزش پسماندم را به اثر سبز اختصاص دادم 🌱\n` +
      `سطح من: ${impact.tier.icon} ${impact.tier.name}`;
    if (navigator.share) {
      try {
        await navigator.share({ text });
        return;
      } catch {
        /* کاربر انصراف داد — بی‌سروصدا به کلیپ‌بورد fallback می‌کنیم */
      }
    }
    if (navigator.clipboard) {
      try {
        await navigator.clipboard.writeText(text);
        setShared(true);
        setTimeout(() => setShared(false), 2500);
      } catch {
        /* کلیپ‌بورد در دسترس نبود — بی‌ضرر نادیده می‌گیریم */
      }
    }
  }

  return (
    <div>
      <TopBar title="اثر سبز من" subtitle="هر تحویل، یک اثر" right={<DemoBadge />} />

      {user && (
        <div className="px-4 mb-1 flex items-center gap-3">
          <span className="w-11 h-11 rounded-full bg-brand-50 text-brand-700 flex items-center justify-center text-base font-bold shrink-0">
            {user.first_name?.[0] || "س"}
          </span>
          <div className="min-w-0">
            <p className="text-sm font-bold text-ink-900 truncate">
              {user.first_name || user.last_name ? `${user.first_name || ""} ${user.last_name || ""}`.trim() : "کاربر سبزینو"}
            </p>
            <p className="text-[11px] text-ink-500">پروفایل اثر سبز شما</p>
          </div>
        </div>
      )}

      {impactLoading ? (
        <CenterLoading />
      ) : !impact ? (
        <EmptyState icon="🌱" title="اطلاعاتی برای نمایش نیست" />
      ) : (
        <div className="px-4 flex flex-col gap-4">
          {/* درخت اثر سبز + سطح/پیشرفت — عنصر اصلی و هویتی صفحه */}
          <div className="rounded-3xl p-5 text-white relative overflow-hidden shadow-md" style={{ background: "linear-gradient(120deg, #0b3d24 0%, #14603a 45%, #1c8a4f 100%)" }}>
            <span className="absolute -left-8 -top-10 w-32 h-32 rounded-full bg-white/10" aria-hidden="true" />
            <span className="absolute right-2 bottom-2 w-24 h-24 rounded-full bg-white/5" aria-hidden="true" />

            <div className="relative z-10 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <span className="w-11 h-11 rounded-2xl bg-white/15 flex items-center justify-center text-2xl">
                  {impact.tier.icon}
                </span>
                <div>
                  <p className="font-extrabold text-base">{impact.tier.name}</p>
                  <p className="text-[11px] text-brand-50/90 mt-0.5">
                    {formatNumber(impact.tier.deliveries)} تحویل ثبت‌شده
                  </p>
                </div>
              </div>
            </div>

            <div className="relative z-10">
              <GreenTree stage={stage} />
            </div>

            {impact.tier.next_name && impact.tier.next_threshold ? (
              <div className="relative z-10">
                <div className="h-2 rounded-full bg-white/20 overflow-hidden">
                  <div className="h-full rounded-full bg-white transition-all" style={{ width: `${tierPct}%` }} />
                </div>
                <p className="text-[10.5px] text-brand-50/90 mt-1.5 text-center">
                  {formatNumber(impact.tier.next_threshold - impact.tier.deliveries)} تحویل دیگر تا «{impact.tier.next_name}»
                </p>
              </div>
            ) : (
              <p className="relative z-10 text-[11px] text-brand-50/90 text-center">🏆 به بالاترین سطح اثر سبز رسیده‌ای!</p>
            )}
            <p className="relative z-10 text-[11px] text-brand-50/80 mt-3 text-center leading-relaxed">
              زباله من، آینده یک نفر 🌍
            </p>
          </div>

          {/* مأموریت بعدی — گیمیفیکیشن */}
          <Card className="p-4 bg-amber-50/70 border border-amber-100">
            <div className="flex items-start gap-3">
              <span className="w-10 h-10 rounded-2xl bg-white flex items-center justify-center text-lg shrink-0 shadow-sm">🎯</span>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-amber-800">مأموریت بعدی تو</p>
                {impact.tier.deliveries === 0 ? (
                  <p className="text-[11px] text-ink-600 mt-1 leading-relaxed">اولین تحویل خودت را ثبت کن و اثر سبز خودت را شروع کن.</p>
                ) : impact.tier.next_threshold ? (
                  <p className="text-[11px] text-ink-600 mt-1 leading-relaxed">
                    {formatNumber(impact.tier.next_threshold - impact.tier.deliveries)} تحویل دیگر تا سطح «{impact.tier.next_name}»
                  </p>
                ) : (
                  <p className="text-[11px] text-ink-600 mt-1 leading-relaxed">همین مسیر را ادامه بده — تو یک قهرمان اثر سبزی!</p>
                )}
                <Link
                  to="/requests/new"
                  className="inline-flex items-center gap-1 text-[11px] font-medium text-amber-800 bg-white px-3 py-1.5 rounded-lg mt-2.5 shadow-sm"
                >
                  ثبت درخواست جدید ‹
                </Link>
              </div>
            </div>
          </Card>

          {/* یادآوری مهلت تخصیص کمک — حداکثر یک هفته پس از وزن‌کشی */}
          {pendingDonations && pendingDonations.length > 0 && (
            <Card className="p-4 bg-rose-50/70 border border-rose-100">
              <p className="text-xs font-bold text-rose-700 mb-2.5">⏳ منتظر تصمیم شماست</p>
              <div className="flex flex-col gap-2">
                {pendingDonations.map((pd) => {
                  const left = daysLeft(pd.deadline);
                  return (
                    <Link
                      key={pd.id}
                      to={`/requests/${pd.request_uid}`}
                      className="flex items-center justify-between bg-white rounded-xl p-3 shadow-sm"
                    >
                      <div className="min-w-0">
                        <p className="text-[11px] font-medium text-ink-900">درخواست #{pd.request_code}</p>
                        <p className="text-[10.5px] text-ink-500 mt-0.5">
                          {left > 0 ? `${formatNumber(left)} روز دیگر مهلت داری تخصیص بدهی` : "مهلت به‌زودی تمام می‌شود"}
                        </p>
                      </div>
                      <span className="text-xs font-bold text-rose-600 shrink-0">{formatToman(pd.amount)} ت</span>
                    </Link>
                  );
                })}
              </div>
              <p className="text-[10px] text-ink-400 mt-2.5 leading-relaxed">
                اگر تا پایان مهلت تخصیص ندهی، این مبلغ خودکار صرف یکی از طرح‌های اثر سبز سبزینو می‌شود.
              </p>
            </Card>
          )}

          {/* Stat grid: kg / economic value / social contribution / points */}
          <div className="grid grid-cols-2 gap-3">
            <Card className="p-4">
              <span className="w-9 h-9 rounded-full bg-sky-50 text-sky-700 flex items-center justify-center text-base mb-2">♻️</span>
              <p className="text-lg font-extrabold text-ink-900">{formatKg(myImpact?.total_kg_recycled || 0)}</p>
              <p className="text-[10.5px] text-ink-500 mt-0.5">کیلوگرم تحویل‌داده‌شده</p>
            </Card>
            <Card className="p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="w-9 h-9 rounded-full bg-violet-50 text-violet-700 flex items-center justify-center text-base">💰</span>
                <InfoTip text="از مجموع وزن‌کشی‌های تکمیل‌شدهٔ شما، بر پایه قیمت روز هر ماده در لحظهٔ تحویل." />
              </div>
              <p className="text-lg font-extrabold text-ink-900">{formatToman(economicValue)}</p>
              <p className="text-[10.5px] text-ink-500 mt-0.5">ارزش اقتصادی ایجادشده (تومان)</p>
            </Card>
            <Card className="p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="w-9 h-9 rounded-full bg-rose-50 text-rose-700 flex items-center justify-center text-base">❤️</span>
                <InfoTip text="جمع مبالغی که خودتان به طرح‌های اثر سبز اختصاص داده‌اید — کاملاً اختیاری و از کیف‌پول شما کسر شده." />
              </div>
              <p className="text-lg font-extrabold text-ink-900">{formatToman(impact.total_contributed)}</p>
              <p className="text-[10.5px] text-ink-500 mt-0.5">مشارکت اثر سبز (تومان)</p>
            </Card>
            <Card className="p-4">
              <span className="w-9 h-9 rounded-full bg-brand-50 text-brand-700 flex items-center justify-center text-base mb-2">🌿</span>
              <p className="text-lg font-extrabold text-ink-900">{points ? formatNumber(points.points) : "—"}</p>
              <p className="text-[10.5px] text-ink-500 mt-0.5">امتیاز سبزینو</p>
            </Card>
          </div>

          {/* اثر واقعی من — شاخص‌های محیط‌زیستی معتبر (همان منبع داده Profile) */}
          {myImpact && (
            <Card className="p-4">
              <div className="flex items-center gap-2 mb-3">
                <p className="text-xs font-bold text-ink-700">🌍 اثر واقعی تو</p>
                {myImpact.is_estimated && (
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200">تخمینی</span>
                )}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="text-center bg-brand-50/60 rounded-xl py-3">
                  <p className="text-base font-extrabold text-brand-700">{formatKg(myImpact.total_kg_recycled)}</p>
                  <p className="text-[10.5px] text-ink-500 mt-0.5">کیلوگرم وارد چرخهٔ بازیافت</p>
                </div>
                <div className="text-center bg-sky-50/60 rounded-xl py-3">
                  <p className="text-base font-extrabold text-sky-700">{formatKg(myImpact.co2_kg_saved_estimated)}</p>
                  <p className="text-[10.5px] text-ink-500 mt-0.5">کیلوگرم CO₂ کاهش‌یافته</p>
                </div>
              </div>
              {myImpact.note && <p className="text-[10.5px] text-ink-400 mt-3 leading-relaxed">{myImpact.note}</p>}
            </Card>
          )}

          {/* Category breakdown */}
          {impact.contributions_count > 0 && (
            <Card className="p-4">
              <p className="text-xs font-bold text-ink-700 mb-3">اثر من به تفکیک حوزه</p>
              <div className="flex flex-col gap-2.5">
                {CATEGORY_ORDER.map((cat) => {
                  const value = impact.category_totals[cat] || 0;
                  const pct = impact.total_contributed > 0 ? Math.round((value / impact.total_contributed) * 100) : 0;
                  if (value <= 0) return null;
                  return (
                    <div key={cat}>
                      <div className="flex items-center justify-between text-[11px] mb-1">
                        <span className="text-ink-700 font-medium">{IMPACT_CATEGORY_ICONS[cat]} {IMPACT_CATEGORY_LABELS[cat]}</span>
                        <span className="text-ink-500">{formatToman(value)} تومان</span>
                      </div>
                      <div className="h-1.5 rounded-full bg-slate-100 overflow-hidden">
                        <div className={`h-full rounded-full ${CATEGORY_BAR_COLORS[cat]}`} style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                  );
                })}
              </div>
            </Card>
          )}

          <Link
            to="/green-impact/projects"
            className="rounded-2xl bg-brand-500 text-white text-sm font-medium py-3.5 text-center shadow-sm active:scale-[0.98] transition"
          >
            🌍 اثر خودت را بزرگ‌تر کن — مشاهده پروژه‌های اثر سبز
          </Link>

          {/* اشتراک‌گذاری اثر سبز */}
          <button
            type="button"
            onClick={handleShare}
            className="rounded-2xl border border-brand-200 text-brand-700 bg-white text-sm font-medium py-3.5 text-center active:scale-[0.98] transition"
          >
            {shared ? "کپی شد ✓ — هرجا خواستی پیست کن" : "📤 اشتراک‌گذاری اثر سبز"}
          </button>

          {/* Contribution history / receipts — به شکل Timeline */}
          <div>
            <p className="text-xs font-bold text-ink-700 mb-3">تاریخچه مشارکت‌ها</p>
            {contributionsLoading ? (
              <CenterLoading />
            ) : !contributions || contributions.length === 0 ? (
              <Card className="p-6 text-center text-xs text-ink-500">
                هنوز مشارکتی ثبت نکرده‌اید. از صفحه «پروژه‌های اثر سبز» می‌توانید شروع کنید.
              </Card>
            ) : (
              <div className="relative pr-4">
                <div className="absolute right-[7px] top-2 bottom-2 w-px bg-brand-100" aria-hidden="true" />
                <div className="flex flex-col gap-4">
                  {contributions.map((c) => (
                    <div key={c.uid} className="relative">
                      <span className="absolute right-[-19px] top-1.5 w-3.5 h-3.5 rounded-full bg-brand-500 border-2 border-white shadow" aria-hidden="true" />
                      <button type="button" onClick={() => setReceipt(c)} className="text-right w-full">
                        <Card className="p-3.5 flex items-center justify-between">
                          <div className="flex items-center gap-2.5 min-w-0">
                            <span className="w-9 h-9 rounded-xl bg-brand-50 flex items-center justify-center text-base shrink-0">
                              {c.project_icon}
                            </span>
                            <div className="min-w-0">
                              <p className="text-xs font-bold text-ink-900 truncate">{c.project_title}</p>
                              <p className="text-[10.5px] text-ink-500 mt-0.5">{toJalali(c.created_at)}</p>
                            </div>
                          </div>
                          <span className="text-sm font-extrabold text-brand-600 shrink-0">{formatToman(c.amount)} ت</span>
                        </Card>
                      </button>
                    </div>
                  ))}
                </div>
              </div>
            )}
          </div>

          <p className="text-[11px] text-ink-400 leading-relaxed pb-2">{impact.note}</p>
        </div>
      )}

      {receipt && <ReceiptModal contribution={receipt} onClose={() => setReceipt(null)} />}
    </div>
  );
}

function InfoTip({ text }: { text: string }) {
  const [open, setOpen] = useState(false);
  return (
    <span className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-5 h-5 rounded-full bg-slate-100 text-ink-500 text-[10px] flex items-center justify-center"
        aria-label="توضیح نحوه محاسبه"
      >
        ℹ️
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-10" onClick={() => setOpen(false)} aria-hidden="true" />
          <div className="absolute left-0 top-6 z-20 w-40 bg-ink-900 text-white text-[10px] leading-relaxed rounded-xl p-2.5 shadow-lg">
            {text}
          </div>
        </>
      )}
    </span>
  );
}
