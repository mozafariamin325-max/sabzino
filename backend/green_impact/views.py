...
from .models import ImpactProject, ImpactContribution, ImpactCategory, PendingDonation, tier_for_deliveries
from .serializers import ImpactProjectSerializer, ImpactContributionSerializer, PendingDonationSerializer
...
class MyPendingDonationsView(generics.ListAPIView):
    """
    تحویل‌های «کمک به اثر سبز»ی که وزن‌کشی شده‌اند اما هنوز شهروند خودش
    تخصیص نداده — برای بنر یادآوری «تا N روز دیگر تخصیص بده» در صفحهٔ اثر
    سبز من. مهلت‌های سررسیدشده اینجا نمایش داده نمی‌شوند چون دستور
    auto_allocate_expired_donations قرار است زودتر آن‌ها را ببندد.
    """

    serializer_class = PendingDonationSerializer
    pagination_class = None
    permission_classes = [permissions.IsAuthenticated]

    def get_queryset(self):
        return PendingDonation.objects.filter(
            request__citizen=self.request.user, resolved=False,
        ).select_related("request").order_by("deadline")
