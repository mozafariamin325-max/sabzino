...
class PendingDonationSerializer(serializers.ModelSerializer):
    request_uid = serializers.CharField(source="request.uid", read_only=True)
    request_code = serializers.CharField(source="request.code", read_only=True)

    class Meta:
        model = PendingDonation
        fields = ("id", "request_uid", "request_code", "amount", "deadline", "resolved", "created_at")
        read_only_fields = fields
