...
def complete_weighing(request_obj: CollectionRequest, material, weight_kg: Decimal, weighed_by=None) -> WeighingRecord:
    ...
    # فاز ۱۴: اگر شهروند در ویزارد «کمک به اثر سبز» را انتخاب کرده بود، مبلغ
    # طبق روال بالا همین الان به کیف‌پول واریز شد (بدون تغییر در رفتار
    # مالی) — این فقط یک مهلت هفت‌روزه برای تخصیص دستی می‌سازد؛ اگر شهروند
    # اقدام نکند، auto_allocate_expired_donations بعداً آن را به طرح
    # پیش‌فرض هدایت می‌کند.
    if request_obj.green_intent == "DONATE":
        from green_impact.services import create_pending_donation

        create_pending_donation(request_obj, total_value)
