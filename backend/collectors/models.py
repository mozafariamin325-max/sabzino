...
class ServiceAreaQuota(TimeStampedModel):
    """
    زیرساخت محدودیت ثبت‌نام راننده بر اساس منطقه — طبق تصمیم محصول، شهروندان
    بدون هیچ محدودیتی ثبت‌نام و تحویل می‌دهند؛ فقط رانندگان طبق ظرفیت هر
    منطقه محدود می‌شوند. فعلاً فقط زیرساخت آماده شده: تا وقتی مدیر برای یک
    منطقه ردیفی اینجا نسازد یا max_collectors را خالی بگذارد، هیچ محدودیتی
    اعمال نمی‌شود (رفتار پیش‌فرض فعلی: کاملاً بدون محدودیت، دقیقاً مثل قبل).
    اعداد واقعی سقف هر منطقه بعداً توسط مدیر از همین پنل ادمین تعیین می‌شود.
    """
    ...
    max_collectors = models.PositiveIntegerField(
        null=True, blank=True, help_text="حداکثر تعداد راننده در وضعیت در انتظار/تأییدشده در این منطقه — خالی یعنی بدون محدودیت",
    )
    is_active = models.BooleanField(default=True)
    ...
    def __str__(self):
        return f"{self.area_name} (حداکثر {self.max_collectors if self.max_collectors is not None else '∞'})"
